...
class AuthMockDataSourceImpl implements AuthRemoteDataSource {
  ...
  Future<List<Map<String, dynamic>>> _loadUsers() async {
    ...
    final jsonString = await rootBundle.loadString(
      'assets/mock_data/auth_responses.json',
    );
  ...
  Future<UserModel> login({
    ...
    final matched = users.cast<Map<String, dynamic>?>().firstWhere((user) {
      ...
      return (email == usernameOrEmail || username == usernameOrEmail) &&
  ...
  Future<UserModel> signup({
    ...
    if (exists) {
      throw _badResponse(
        statusCode: 409,
        message: 'البريد الإلكتروني أو اسم المستخدم موجود مسبقا',
      );
    }
  ...
  Future<void> resetPassword({
    ...
    if (expectedCode == null) {
      throw _badResponse(
        statusCode: 400,
        message: 'يرجى طلب إعادة تعيين كلمة المرور أولا',
      );
    }
    ...
    final index = users.indexWhere(
      (u) => u['email']?.toString().toLowerCase() == emailKey,
    );
    if (index == -1) {
      throw _badResponse(
        statusCode: 404,
