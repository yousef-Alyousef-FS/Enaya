...
class _PatientAppointmentHistoryScreenState extends State<PatientAppointmentHistoryScreen> {
          ...
          Expanded(
            child: BlocBuilder<PatientAppointmentsCubit, PatientAppointmentsState>(
              builder: (context, state) {
                ...
                if (state.status == PatientAppointmentsStatus.loading && history.isEmpty) {
                ...
                return RefreshIndicator(
                  onRefresh: () => context.read<PatientAppointmentsCubit>().loadAppointments(PatientSession().patientId ?? 'p1'),
                  child: ListView.builder(
                    controller: _scrollController,
                    padding: const EdgeInsets.all(16),
                    itemCount: history.length + (state.isPageLoading ? 1 : 0),
                    itemBuilder: (context, index) {
                      ...
                      return Padding(
                        ...
                        child: AppointmentCard(
                          ...
                          onTap: () => _openDetails(context, history[index]),
                        ),
                      );
                    },
                  ),
  ...
  Widget _buildFilterBar(ThemeData theme) {
    return Container(
      ...
      child: ListView(
        ...
        children: [
          _FilterChip(label: 'all'.tr(), isSelected: _statusFilter == null, onTap: () => setState(() => _statusFilter = null)),
          _FilterChip(label: 'completed'.tr(), isSelected: _statusFilter == AppointmentStatus.completed, onTap: () => setState(() => _statusFilter = AppointmentStatus.completed)),
          _FilterChip(label: 'cancelled'.tr(), isSelected: _statusFilter == AppointmentStatus.cancelled, onTap: () => setState(() => _statusFilter = AppointmentStatus.cancelled)),
        ],
      ),
    );
  }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('no_appointments_found'.tr(), style: const TextStyle(color: Colors.grey)),
        ],
      ),
    );
  }
  ...
  void _openDetails(BuildContext context, dynamic appointment) {
    context.push('/appointments/details', extra: {
...
class _FilterChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: ChoiceChip(label: Text(label), selected: isSelected, onSelected: (_) => onTap(), selectedColor: theme.colorScheme.primaryContainer),
    );
  }
}
